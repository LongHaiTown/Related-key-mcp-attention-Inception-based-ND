...
def evo(f, n=NUM_GENERATIONS, num_bits=32, L = 128, gen=None, verbose = 0, rounds=7):
    """Evolutionary search (evo) — the sole heuristic retained.

    Generates initial population of size L**2, iteratively produces offspring via
    pairwise XOR, applies random bit-flip mutation, filters duplicates, and keeps
    the top-L individuals by score each generation.
    """
    mutProb = 50
    if gen is None:
...
# Removed alternative heuristics (pso, ga, de, aco, sa, gwo) to keep code minimal and focused on evo.
...
def optimize(plain_bits, key_bits, encryption_function, nb_samples=NUM_GENERATIONS, scenario = "single-key", log_file = None, epsilon=0.1, rounds=7):
    allDiffs = None
    totalScores = {}
    diffs = None
    ...
    for current_round in range(1, rounds + 1):
        ...
        diffs, scores = evo(
            ...
            num_bits=bits_to_search, L=32, gen=diffs, verbose=1, rounds=rounds)
        if allDiffs is None:
            allDiffs = diffs
        else:
