...
def ECA_Module(input_tensor, gamma=2, b=1):
    ...
    gap = GlobalAveragePooling1D(name='eca_gap')(input_tensor)  # shape: (B, C)
    ...
    x = Reshape((channel, 1), name='eca_reshape_gap')(gap)  # shape: (B, C, 1)
    x = Conv1D(1, kernel_size=k_size, padding='same', use_bias=False, name='eca_conv')(x)  # shape: (B, C, 1)
    x = Activation('sigmoid', name='eca_sigmoid')(x)

    # Step 4: Reweight input_tensor
    x = Reshape((1, channel), name='eca_reshape_weights')(x)  # shape: (B, 1, C)
    output = Multiply(name='eca_reweight')([input_tensor, x])  # broadcasting
...
def make_model_inception_no_eca(pairs: int = 2, plain_bits: int = 32):
    """
    Inception-based Neural Distinguisher (Ablation: without ECA attention)

    Same architecture as make_model_inception but skips the ECA attention module.
    """
    if plain_bits % 2 != 0:
        raise ValueError("plain_bits must be divisible by 2 for model reshaping (got %d)" % plain_bits)
