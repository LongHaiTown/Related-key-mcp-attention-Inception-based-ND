...
def main():
    ...
    # Allocate arrays (optional; kept for compatibility, not strictly required for CSV logging)
    eigenvalue_ratios = np.zeros((num_diffs, input_dim_sweep), dtype=float)
    eigenvalue_vectors = np.zeros((num_diffs, input_dim_sweep, input_dim_sweep), dtype=float)
    true_labels_store = np.zeros((num_diffs, datasize_sweep), dtype=np.uint8)
    cluster_labels_store = np.zeros((num_diffs, datasize_sweep), dtype=np.int32)
    silhouette_scores_store = np.zeros(num_diffs)
    ...
    print("=" * 70)
    print(
        f"✅ Sweep complete! Total time: {elapsed_sweep:.1f}s ({elapsed_sweep/60:.1f} minutes)"
    )
    print("Results saved to:", results_csv)
    print("=" * 70)

    # === Also export sorted CSVs by key metrics (descending) ===
    # Metrics: biased_pcs, max_diff, silhouette_clusters, silhouette_true
